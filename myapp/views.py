...
def findbus(request):
    ...
    if request.method == 'POST':
        ...
        pickup_point_r=request.POST.get('pickup point')
        date_r = request.POST.get('date')
        bus_list = Bus.objects.filter(source=source_r, destination=destination_r, date=date_r)
        if bus_list:
    ...
    if request.method == 'POST':
        ...
        if bus:
            if bus.rem > int(seats_r):
                ...
                source_r = bus.source
                destination_r = bus.destination
                pickup_point_r = bus.pickup_point
                price_r = bus.price
                date_r = bus.date
                time_r = bus.time
                ...
                book = Book.objects.create(name=username_r, email=email_r, userid=userid_r, bus_name=name_r,
                                           ...
                                           destination= destination_r, price=price_r, date=date_r, time=time_r,
                                           status='BOOKED')
...
def signout(request):
    context = {}
    logout(request)
    context['error'] = "You have succesfully loged out"
    return render(request, 'myapp/signin.html', context)
